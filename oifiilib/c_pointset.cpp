...
//2021fev08, spi, begin
WCHAR* pwcharStatChannelNames[POINTSET_DEFAULT_NUMBEROFSTAT + POINTSET_EXTRA_NUMBEROFSTAT] =
{
	L"VoroArea",
	L"VoroDens",
	L"AvgVoroArea",	//1NeiAvgVoroArea
	L"AvgTreeInt",	//1NeiAvgIntensity
	L"AvgTreeArea",	//1NeiAvgTreeArea
	L"VarVoroArea",	//1NeiVarVoroArea
	L"VarIntensity",	//1NeiVarIntensity
	L"VarTreeArea",	//1NeiVarTreeArea
	L"ClassTreeInt",
	L"TreeInt",
	L"TreeDimX",
	L"TreeDimY",
...
CTime prevNewPointsetTimeStamp(CTime::GetCurrentTime());
DWORD prevnewpointsetstamp_ms = GetTickCount();
int pointset_debug_filename_nnn = 0;
//2021fev08, spi, end
...
POINTSET* NewPointset(long maxnumberofelements)
{
	...
	//CTimeSpan DiffTime = NowTime - prevNewPointsetTimeStamp;
	if ((nowstamp_ms - prevnewpointsetstamp_ms) < 1000)
	{
		//2021nov19, spi, begin
		/*
		//if sub second, use milliseconds
		if ((nowstamp_ms - prevnewpointsetstamp_ms) < 1)
		{
	...
	//YYYYMMDD_HHMMSS_mmm_nnn
	CString prefixedfilename = NowTime.Format("%Y%b%d_%H%M%S_");
	char charbuf[256];
	sprintf(charbuf, "%03d", pointset_debug_filename_nnn);
	prefixedfilename = prefixedfilename + charbuf + POINTSET_DEBUG_FILENAME;
	pPointset->pFILE_debug = NULL;
	if (POINTSET_DEBUG)
	{
		pPointset->pFILE_debug = _wfopen(prefixedfilename.GetString(), L"w");
	}
	...
	{
		printf("not able to allocate POINTSET structure (controlratio)\n");
		free(pPointset->py);
		free(pPointset->px);
		free(pPointset->cty);
		free(pPointset->ctx);
	...
	//spi, end
	//spi, begin
	pPointset->vibrationangle = (float*)malloc(maxnumberofelements*sizeof(float));
	if (pPointset->vibrationangle == NULL)
	{
		printf("not able to allocate POINTSET structure (py)\n");
		free(pPointset->py);
		free(pPointset->px);
	...
	pPointset->color_r = (float*)malloc(maxnumberofelements*sizeof(float));
	if (pPointset->color_r == NULL)
	{
		printf("not able to allocate POINTSET structure (py)\n");
		free(pPointset->vibrationangle);
		free(pPointset->py);
		free(pPointset->px);
		free(pPointset->cty);
		free(pPointset->ctx);
	...
	pPointset->color_g = (float*)malloc(maxnumberofelements*sizeof(float));
	if (pPointset->color_g == NULL)
	{
		printf("not able to allocate POINTSET structure (py)\n");
		free(pPointset->color_r);
		free(pPointset->vibrationangle);
		free(pPointset->py);
		free(pPointset->px);
		free(pPointset->cty);
	...
	pPointset->color_b = (float*)malloc(maxnumberofelements*sizeof(float));
	if (pPointset->color_b == NULL)
	{
		printf("not able to allocate POINTSET structure (py)\n");
		free(pPointset->color_g);
		free(pPointset->color_r);
		free(pPointset->vibrationangle);
		free(pPointset->py);
		free(pPointset->px);
	...
	pPointset->color_a = (float*)malloc(maxnumberofelements*sizeof(float));
	if (pPointset->color_a == NULL)
	{
		printf("not able to allocate POINTSET structure (py)\n");
		free(pPointset->color_b);
		free(pPointset->color_g);
		free(pPointset->color_r);
		free(pPointset->vibrationangle);
		free(pPointset->py);
...
LONG ReallocPointset(POINTSET* pPointset, long new_maxnumberofelements)
{
	...
	*/
	...
	//spi, end
	//spi, begin
	pPointset->vibrationangle = (float*)realloc(pPointset->vibrationangle, new_maxnumberofelements*sizeof(float));
	if (pPointset->vibrationangle == NULL)
	{
		printf("not able to allocate POINTSET structure (vibrationangle)\n");
		free(pPointset->py);
		free(pPointset->px);
		free(pPointset->cty);
		free(pPointset->ctx);
	...
	pPointset->color_r = (float*)realloc(pPointset->color_r, new_maxnumberofelements*sizeof(float));
	if (pPointset->color_r == NULL)
	{
		printf("not able to allocate POINTSET structure (color_r)\n");
		free(pPointset->vibrationangle);
		free(pPointset->py);
		free(pPointset->px);
		free(pPointset->cty);
	...
	pPointset->color_g = (float*)realloc(pPointset->color_g, new_maxnumberofelements*sizeof(float));
	if (pPointset->color_g == NULL)
	{
		printf("not able to allocate POINTSET structure (color_g)\n");
		free(pPointset->color_r);
		free(pPointset->vibrationangle);
		free(pPointset->py);
		free(pPointset->px);
	...
	pPointset->color_b = (float*)realloc(pPointset->color_b, new_maxnumberofelements*sizeof(float));
	if (pPointset->color_b == NULL)
	{
		printf("not able to allocate POINTSET structure (color_b)\n");
		free(pPointset->color_g);
		free(pPointset->color_r);
		free(pPointset->vibrationangle);
		free(pPointset->py);
	...
	pPointset->color_a = (float*)realloc(pPointset->color_a, new_maxnumberofelements*sizeof(float));
	if (pPointset->color_a == NULL)
	{
		printf("not able to allocate POINTSET structure (color_a)\n");
		free(pPointset->color_b);
		free(pPointset->color_g);
		free(pPointset->color_r);
		free(pPointset->vibrationangle);
...
{
	//2021fev08, spi, begin
	if (pPointset->pFILE_debug) fclose(pPointset->pFILE_debug);
	//2021fev08, spi, end
...
POINTSET* NewPointsetCopy(POINTSET* pPointset)
{
	...
	//spi, end
	//spi, begin
	memcpy(pNewPointset->vibrationangle, pPointset->vibrationangle, sizeof(float)*pPointset->maxnumberofelements);
	memcpy(pNewPointset->color_r, pPointset->color_r, sizeof(float)*pPointset->maxnumberofelements);
	memcpy(pNewPointset->color_g, pPointset->color_g, sizeof(float)*pPointset->maxnumberofelements);
	memcpy(pNewPointset->color_b, pPointset->color_b, sizeof(float)*pPointset->maxnumberofelements);
	memcpy(pNewPointset->color_a, pPointset->color_a, sizeof(float)*pPointset->maxnumberofelements);
	//spi, end
...
//2021nov29, spi, begin
void ScalePointset(POINTSET* pPointset, double x_scale/* = 1.0f*/, double y_scale/* = 1.0f*/)
{
	if (pPointset == NULL)
	{
		ASSERT(FALSE);
...
void TranslatePointset(POINTSET* pPointset, double x_offset, double y_offset, double x_scale/* = 1.0f*/, double y_scale/* = 1.0f*/)
{
	if(pPointset==NULL)
	{
		ASSERT(FALSE);
...
void BuildTriangleNetwork(POINTSET* pPointset)
{
	...
	if (pPointset && pPointset->pFILE_debug)
	{
		fprintf(pPointset->pFILE_debug, "BuildTriangleNetwork() start ... \n\n");
	}
    pPointset->loptim=0;
    strip=0.05;
	...
	//2021fev08, spi, begin
	if (POINTSET_SKIP_THE_TWO_FSORT_CALLS)
	{
		//nowadays, using oifii on large 12K images, optimization, 
		//like kicking out the two costly fsort calls, becomes mandatory
	}
	else
	{
		...
		*/
		...
		if (pPointset && pPointset->pFILE_debug)
		{
			fprintf(pPointset->pFILE_debug, "  first sort ...\n");
		}
		fsort(pPointset, pPointset->vt[2], 0, i - 1, xcmp);
		if (pPointset && pPointset->pFILE_debug)
		{
			fprintf(pPointset->pFILE_debug, "  done.\n\n");
		}
		...
		*/
		if (pPointset && pPointset->pFILE_debug)
		{
			fprintf(pPointset->pFILE_debug, "  second sort ...\n");
		}
	...
	if (pPointset && pPointset->pFILE_debug)
	{
		fprintf(pPointset->pFILE_debug, "  convex hull frontier creation start ...\n");
		fflush(pPointset->pFILE_debug);
	}
    ...
    for (i=2; i<pPointset->npts; i++)
    {
		...
		//re-inserting march 1997 fix, because spivoronoimidiwin32 crashes
		/*
		...
		//while((ckturn(pPointset,pPointset->nt[0],nptf,i) > 0) && (nptf >= 1)) //original prior to 1997mar27
		//2022may17, spi, begin
		//removing march 1997 fix to experiment
		//while((nptf >= 1) && (ckturn(pPointset,pPointset->nt[0],nptf,i) > 0)) //1997mar27, fix 
		while((ckturn(pPointset,pPointset->nt[0],nptf,i) > 0) && (nptf >= 1)) //original prior to 1997mar27
		//2022may17, spi, end
		*/
		while((nptf >= 1) && (ckturn(pPointset,pPointset->nt[0],nptf,i) > 0)) //1997mar27, fix 
    ...
    {
		pPointset->vt[0][i]=pPointset->dcd[0]; //2022may17, spi, should it not be dcd[i+0] instead? cannot validate
	...
	if (pPointset && pPointset->pFILE_debug)
	{
		fprintf(pPointset->pFILE_debug, "  convex hull frontier creation done.\n");
		fprintf(pPointset->pFILE_debug, "  start computing triangulation ...\n");
		fflush(pPointset->pFILE_debug);
	}
	...
	/*
	CLogDocument* pLogDocument = ((COIIIApp*)AfxGetApp())->GetLogDocument();
	if (pLogDocument && POINTSET_DEBUG)
	{
		CString myString;
		myString.Format(L"nombre de points qui coincident =  %d\r\n", nrej2);
		pLogDocument->AddTextW(myString.GetString());
		myString.Format(L"nombre de points rejetes        =  %d\r\n", nrej1);
		pLogDocument->AddTextW(myString.GetString());
		myString.Format(L"nombre de triangles construits  =  %d\r\n", pPointset->ntri + 1);
		pLogDocument->AddTextW(myString.GetString());
	}
	*/
	if (pPointset->pFILE_debug)
	{
		fprintf(pPointset->pFILE_debug, "  done computing triangulation.\n\n");
		fprintf(pPointset->pFILE_debug, "  nombre de points qui coincident =  %d\n", nrej2);
		fprintf(pPointset->pFILE_debug, "  nombre de points rejetes        =  %d\n", nrej1);
		fprintf(pPointset->pFILE_debug, "  nombre de triangles construits  =  %d\n", pPointset->ntri + 1);
		fprintf(pPointset->pFILE_debug, "BuildTriangleNetwork() done.\n\n");
		fflush(pPointset->pFILE_debug);
	}
	//2021fev08, spi, end
    return;
		  ...
		  int k1)
{
	...
	int nea;
	int i,i1,k2,k3;
	if (pPointset->loptim == MAXOPT)
	{
		//2022may17, spi, begin
		if (pPointset && pPointset->pFILE_debug)
		{
			fprintf(pPointset->pFILE_debug, "niveau maximal d'optimisation atteint, dans optim()\n");
			fflush(pPointset->pFILE_debug);
		}
		//2022may17, spi, end
		return TRUE;
	...
	//pPointset->loptim++; //2022jan04, spi, ++ was separated from the variable
	pPointset->loptim = (pPointset->loptim)+1; 
	//2022may16, spi, begin
	//for (i=0; i<3; i++) if(pPointset->nt[i][nea] == it ) break;
	for (i = 0; i < 3; i++)
	{
		if (pPointset->nt[i][nea] == it) break;
	}
	//2022may16, spi, end
	if (i == 3)
	{
		if (pPointset && pPointset->pFILE_debug)
		{
			fprintf(pPointset->pFILE_debug, "erreur fatale dans optim()\n");
			fflush(pPointset->pFILE_debug);
		}
...
//2021fev08, spi, begin
//added to validate the tin construction
//since we now have the option of kicking 
//out the two initial fsort() calls just
//before building the convex hull. 
bool IsTriangleStoredCounterClockwise(POINTSET* pPointset, int idtri)
{
	bool bresult = false;
	double area = 0.0;

	// Calculate value of shoelace formula
	int n = 3; //for a triangle 
	int j = n - 1;
	for (int i = 0; i < n; i++)
	{
		//area += (X[j] + X[i]) * (Y[j] - Y[i]); 
		area += (pPointset->px[pPointset->vt[j][idtri]] + pPointset->px[pPointset->vt[i][idtri]]) * (pPointset->py[pPointset->vt[j][idtri]] - pPointset->py[pPointset->vt[i][idtri]]);
		j = i;  // j is previous vertex to i 
	}

	//return abs(area / 2.0);
	if (area > 0.0) bresult = true; //yes triangle is counter-clockwise
	return bresult;
}
//2021fev08, spi, end
...
void ComputeTriangleCenter(POINTSET* pPointset, int idtri)
{
	...
	else
	{
		if (pPointset && pPointset->pFILE_debug)
		{
			fprintf(pPointset->pFILE_debug, "error in ComputeTriangleCenter(), three points too close\n");
			fflush(pPointset->pFILE_debug);
		}
						   ...
						   int* p_itriseed)
{
    ...
    if(p_itriseed == NULL) 
	{
		...
		*/
		if (pPointset && pPointset->pFILE_debug)
		{
			fprintf(pPointset->pFILE_debug, "error in FindTriContainingPoint(), p_itriseed is null\n");
			fflush(pPointset->pFILE_debug);
		}
		return -99;
	}
	...
	if(p_itriseed[0]>pPointset->ntri)
	{
		if (pPointset && pPointset->pFILE_debug)
		{
			fprintf(pPointset->pFILE_debug, "warning in FindTriContainingPoint(), itriseed found to be exceeding pPointset->ntri, safely resetted to 0\n");
			fflush(pPointset->pFILE_debug);
		}
		/* //spi 2014
		...
		for (i=0; i<=2; i++)
		{
			...
			if (area <0.)
			{
				if(pPointset->nt[i1][newtr] == -1)
				{
					if (pPointset && pPointset->pFILE_debug)
					{
						fprintf(pPointset->pFILE_debug, "warning in FindTriContainingPoint(), no triangle found\n");
						fflush(pPointset->pFILE_debug);
					}
			...
			else if (area == 0.)
			{
				if( x31 == 0. && y31 == 0.)
				{
					...
					if (pPointset && pPointset->pFILE_debug)
					{
						fprintf(pPointset->pFILE_debug, "FindTriContainingPoint(), (xa,ya) coincides with a triangle vertex\n");
						fflush(pPointset->pFILE_debug);
					}
					*/
					p_itriseed[0] = newtr;
								...
								int* p_arraytri)
{
	if(pPointset==NULL || p_itriseed==NULL || p_numtrifound==NULL || p_arraytri==NULL)
	{
		if (pPointset && pPointset->pFILE_debug)
		{
			fprintf(pPointset->pFILE_debug, "error in FindTriContainingPoint_CAT(), at least one of the pointers pass on call is null\n");
			fflush(pPointset->pFILE_debug);
		}
		ASSERT(FALSE);
		return -99;
    ...
    if(p_itriseed == NULL) 
	{
		if (pPointset && pPointset->pFILE_debug)
		{
			fprintf(pPointset->pFILE_debug, "error in FindTriContainingPoint_CAT(), p_itriseed is null\n");
			fflush(pPointset->pFILE_debug);
		}
		/* //spi 2014
		CLogDocument* pLogDocument = ((COIIIApp*)AfxGetApp())->GetLogDocument();
	...
	if(p_itriseed[0]>pPointset->ntri)
	{
		if (pPointset && pPointset->pFILE_debug)
		{
			fprintf(pPointset->pFILE_debug, "warning in FindTriContainingPoint_CAT(), itriseed found to exceed ntri and has been resetted to 0\n");
			fflush(pPointset->pFILE_debug);
		}
		/* //spi 2014
		CLogDocument* pLogDoc = ((COIIIApp*)AfxGetApp())->GetLogDocument();
		...
		*/
		...
		if (area[i]==0.)
		{
			...
			if( x31[i]==0. && y31[i]==0.)
			{
				...
				if (pPointset && pPointset->pFILE_debug)
				{
					fprintf(pPointset->pFILE_debug, "FindTriContainingPoint_CAT(), (xa,ya) coincides with a triangle vertex\n");
					fflush(pPointset->pFILE_debug);
				}
				*/
				p_itriseed[0] = newtr;
		...
		else if (area[i] <0.)
		{
			if(pPointset->nt[i_adj[i]][newtr] == -1)
			{
				if (pPointset && pPointset->pFILE_debug)
				{
					fprintf(pPointset->pFILE_debug, "warning in FindTriContainingPoint_CAT(), no triangle found\n");
					fflush(pPointset->pFILE_debug);
				}
				// store the last triangle found before going
				//   outside the convex frontier 
...
int FindNearestNeighbor(POINTSET* pPointset,
			...
			//int* p_itriseed)
			int* p_itriseed,
			int adjacenttriflag/*=POINTSET_ADJACENTTRIFLAG_NOADJTRI*/) //=0 to exclude any adjacent tri, =1 to include one adjacent tri, =3 to include all three adjacent triangles
//2022may18, spi, end
{
    ...
    if(p_itriseed == NULL) 
	{
		if (pPointset && pPointset->pFILE_debug)
		{
			fprintf(pPointset->pFILE_debug, "error in FindNearestNeighbor(), p_itriseed is null\n");
			fflush(pPointset->pFILE_debug);
		}
    ...
    if( itrianglefound >= 0)
    {
		for(i=0; i<3; i++)
		{
		    ...
		    r2[i] = (xa-pPointset->px[index])*(xa-pPointset->px[index])+(ya-pPointset->py[index])*(ya-pPointset->py[index]);
		    if(i==0 || r2[i]<r2min)
		    {
				r2min = r2[i];				
				inearest = index;
		...
		//2022may18, spi, begin
		if (adjacenttriflag>0)
		{
			//include one adjacent triangle
			int idvertex_nearest = inearest;
			int idvertex_secondnearest = -1; //find it
			//since we have stored r2 for each vertex
			for (i = 0; i < 3; i++)
			{
				if (r2[i]==r2min)  //when i correspond to the triangle's closest vertex
				{
					//the two other triangle's farer vertices are
					int ia = ijk[i + 1];
					int ib = ijk[i + 2];
					if (r2[ia] < r2[ib])
					{
						//vertex a is the second nearest
						idvertex_secondnearest = pPointset->vt[ia][itrianglefound];
					}
					else
					{
						//vertex b is the second nearest
						idvertex_secondnearest = pPointset->vt[ib][itrianglefound];
					}
					break;
				}
			}
			//include one adjacent triangle
			if (idvertex_secondnearest >= 0)
			{
				//now, find the adjacent triangle, using idvertex_nearest, idvertex_secondnearest, and itrianglefound
				int idtri_adjacent = GetAdjTri(pPointset, idvertex_nearest, idvertex_secondnearest, itrianglefound);
				if (idtri_adjacent >= 0)
				{
					int idvertex_adjacent = GeThirdVertex(pPointset, idvertex_nearest, idvertex_secondnearest, idtri_adjacent);
					if (idvertex_adjacent >= 0)
					{
						r2_adjacent = (xa - pPointset->px[idvertex_adjacent])*(xa - pPointset->px[idvertex_adjacent]) + (ya - pPointset->py[idvertex_adjacent])*(ya - pPointset->py[idvertex_adjacent]);
						if (r2_adjacent < r2min)
						{
							return idvertex_adjacent; //adjacent triangle's third vertex is the nearest
						}
					}
				}
			}
		...
		//2022may18, spi, end
		return inearest; //nearest vertex from initial triangle
    }
		...
		*/
		...
		for(i=0; i<3; i++)
		{
		    ...
		    if( (xa-pPointset->px[index])==0.0 && (ya-pPointset->py[index])==0.0 )
			{
				if (pPointset && pPointset->pFILE_debug)
				{
					fprintf(pPointset->pFILE_debug, "FindNearestNeighbor(), (xa,ya) coincides precisely with a vertex\n");
					fflush(pPointset->pFILE_debug);
				}
				inearest = index;
				return inearest;
		...
		if (pPointset && pPointset->pFILE_debug)
		{
			fprintf(pPointset->pFILE_debug, "error in FindNearestNeighbor(), (xa,ya) should coincides but did not precisely match with any vertex\n");
			fflush(pPointset->pFILE_debug);
		}
    ...
    else if(itrianglefound==-1)
    {
		if (pPointset && pPointset->pFILE_debug)
		{
			fprintf(pPointset->pFILE_debug, "error in FindNearestNeighbor(), (xa,ya) is outside the convex hull\n");
			fflush(pPointset->pFILE_debug);
		}
	...
	if (pPointset && pPointset->pFILE_debug)
	{
		fprintf(pPointset->pFILE_debug, "developpment-time error in FindNearestNeighbor(), (xa,ya) is outside the convex hull?\n");
		fflush(pPointset->pFILE_debug);
	}
	return -1;
}
...
// should be used.
///////////////////////////////////////////////////////////////////
int InvalidTri( POINTSET* pPointset, int iadjtri)
{
    // case 1 
	...
	*/
	...
	//2022july23, spi, begin
	//if (1) //inserting it back, on 2021dec27
	//if (0) //kicking it out once again, on 2022july23, for spivoronoimidiwin32 app so everything gets drawn within the bounding rect, otherwise it leaves black all triangles overlapping outside bounding rect 
	if (1) //inserting it back, on 2022july23, for spivoronoimidiwin32 app to work properly
	//2022july23, spi, end
	{
		// case 2 
		if (pPointset->ctx[iadjtri] < pPointset->xmin ||
			...
			pPointset->cty[iadjtri] > pPointset->ymax)
		{
			//if (pPointset && pPointset->pFILE_debug)
			//{
			//	fprintf(pPointset->pFILE_debug, "warning in InvalidTri(), triangle center outside bounding rect\n");
			//	fflush(pPointset->pFILE_debug);
			//}
			return TRUE; //this tri is invalid
		}
	}
	//2021dec27, spi, end
	return FALSE; //this tri is valid
}
				 ...
				 int* p_arrayneighbor )
{
	...
	|| p_arraytri == NULL || p_arrayneighbor == NULL)
    {
		if (pPointset && pPointset->pFILE_debug)
		{
			fprintf(pPointset->pFILE_debug, "error in FindAllTriSurroundingVertex(), at least one of the supplied pointers is null\n");
			fflush(pPointset->pFILE_debug);
		}
    ...
    if(ifirstfoundtri != -2)
    {
		...
		*/
		if (pPointset && pPointset->pFILE_debug)
		{
			fprintf(pPointset->pFILE_debug, "error in FindAllTriSurroundingVertex(), ifirstfoundtri is not -2\n");
			fprintf(pPointset->pFILE_debug, "error in FindAllTriSurroundingVertex(), should get -2 because the input point is a vertex, and a vertex is never inside a triangle, it is a part of a triangle. the only time a vertex could\n");
			fprintf(pPointset->pFILE_debug, "error in FindAllTriSurroundingVertex(), be inside a existing triangle is when we just inserted this new vertex and the triangulation needs update.\n");
			fflush(pPointset->pFILE_debug);
		}
		return -99;
    }
    ...
    if(ifirstfoundtri == -1)
    {
		if (pPointset && pPointset->pFILE_debug)
		{
			fprintf(pPointset->pFILE_debug, "error in FindAllTriSurroundingVertex(), ifirstfoundtri is -1 but should not, cannot find surrounding triangles for boundary vertices.\n");
			fflush(pPointset->pFILE_debug);
		}
    ...
    if( InvalidTri(pPointset, ifirstfoundtri) )
    {
		...
		do
		{
		    ...
		    if(i==3)
		    {
				if (pPointset && pPointset->pFILE_debug)
				{
					fprintf(pPointset->pFILE_debug, "error in FindAllTriSurroundingVertex(), ivertex %d not found in iadjtri %d\r\n", ivertex, iadjtri);
					fflush(pPointset->pFILE_debug);
				}
		    ...
		    if( iadjtri==ifirstfoundtri )
		    {
				if (pPointset && pPointset->pFILE_debug)
				{
					fprintf(pPointset->pFILE_debug, "error in FindAllTriSurroundingVertex(), cannot find one valid tri for vertex (ccw search)\r\n");
					fflush(pPointset->pFILE_debug);
				}
		...
		if( iadjtri == -1 )
		{
		    ...
		    do
		    {
				...
				if( iadjtri==ifirstfoundtri )
				{
					if (pPointset && pPointset->pFILE_debug)
					{
						fprintf(pPointset->pFILE_debug, "error in FindAllTriSurroundingVertex(), cannot find one valid tri for vertex (ccw + cw search)\r\n");
						fflush(pPointset->pFILE_debug);
					}
    ...
    do
    {
		...
		if(iadjtri == -1 || InvalidTri(pPointset, iadjtri) )
		{
			if (0)
			{
				if (pPointset && pPointset->pFILE_debug)
				{
					fprintf(pPointset->pFILE_debug, "error in FindAllTriSurroundingVertex(),  In searching for next counter-clockwise adjacent triangle we get an invalid triangle (index=-1 or tri center outside convexe envelope bounding rect)\r\n");
					fflush(pPointset->pFILE_debug);
				}
			}
		...
		if(iadjtri == -99)
		{
			if (pPointset && pPointset->pFILE_debug)
			{
				fprintf(pPointset->pFILE_debug, "error in FindAllTriSurroundingVertex(), inconsistant vertices pair (%d,%d) for triangle %d\r\n", ivertex, ineighbor, p_arraytri[itri]);
				fflush(pPointset->pFILE_debug);
			}
...
//2022may18, spi, begin
int GeThirdVertex(POINTSET* pPointset,
	int ivertex1,
	int ivertex2,
	int itri)
{
	int ivertex3 = -1;
	if ( (itri>=0) && (ivertex1!=ivertex2) )
	{
		int i1 = -1;
		int i2 = -1;
		int i3 = -1;
		for (int i=0; i<3; i++)
		{
			if (pPointset->vt[i][itri] == ivertex1)
			{
				i1 = i;
			}
			else if (pPointset->vt[i][itri] == ivertex2)
			{
				i2 = i;
			}
		}
...
//2021nov29, spi, begin
int IsPointOnLineSegment(double x1, double y1, double x2, double y2, double x, double y, double x_res/*=1.0*/, double y_res/*=1.0*/) //line segment
{
	//bounding box
	double bb_xmax = max(x1, x2);
	double bb_xmin = min(x1, x2);
	double bb_ymax = max(y1, y2);
	double bb_ymin = min(y1, y2);
	if ( (x < (bb_xmin-x_res)) || (x > (bb_xmax+x_res))) return false;
	if ( (y < (bb_ymin-y_res)) || (y > (bb_ymax+y_res))) return false;

	//let's find MB, y = mx + b
	double m = 0.0;
	double b = 0.0;
	...
	if (result)
	{
		...
		double det = y - m * x - b;
		double det1 = ymin - m * xmin - b;
		double det2 = ymin - m * xmax - b;
		double det3 = ymax - m * xmin - b;
		double det4 = ymax - m * xmax - b;
		double detmin = min(det, det1);
		detmin = min(det, det2);
		detmin = min(det, det3);
		detmin = min(det, det4);
		double detmax = max(det, det1);
		detmax = max(det, det2);
		detmax = max(det, det3);
		detmax = max(det, det4);
		...
		{
			//to place a break point when debuging
			int dummy = 0;
		}
	}
	else
